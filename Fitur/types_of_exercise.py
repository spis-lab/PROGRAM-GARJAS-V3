import numpy as np
from Fitur.body_part_angle import BodyPartAngle
...
class TypeOfExercise(BodyPartAngle):
    ...
    def push_up(self, counter, status):
        ...
        spine = detection_body_part(self.landmarks, "MID_HIP")  

        if status:
            if avg_arm_angle < 70 and avg_shoulder < avg_shoulder_y and spine[1] < avg_shoulder_y:  
                counter += 1
                status = False
        else:
    ...
    def pull_up(self, counter, status):
        ...
        avg_wrist = (left_wrist[1] + right_wrist[1]) / 2
        left_elbow = detection_body_part(self.landmarks, "LEFT_ELBOW")
        right_elbow = detection_body_part(self.landmarks, "RIGHT_ELBOW")
        avg_shoulder_y = (left_elbow[1] + right_elbow[1]) / 2
        eye = detection_body_part(self.landmarks,"LEFT_EYE") 
    ...
    def sit_up(self, counter, status):
        ...
        elbow = detection_body_part(self.landmarks, "LEFT_ELBOW") 

        if status:
            if angle < 50 and elbow[1] >= knee[1]:  
                counter += 1
                status = False
        else:
            if angle > 110:
